...
#define B_FLAG_FORCE_DOUBLE_WILD    FLAG_SYS_FORCE_DOUBLE_WILD // If this flag is set, all land and surfing wild battles will be double battles.
#define B_SMART_WILD_AI_FLAG        FLAG_SYS_SMART_WILD_AI     // If not 0, you can set this flag in a script to enable smart wild pokemon
...
#define B_FLAG_TERA_ORB_CHARGED     FLAG_SYS_TERA_ORB_CHARGED  // If this flag is set, the Tera Orb is charged. It is automatically set upon healing and cleared upon Terastallizing once configured.
#define B_FLAG_TERA_ORB_NO_COST     FLAG_SYS_TERA_ORB_NO_COST  // If this flag is set, the Tera Orb does not use up its charge upon Terastallization. In S/V, this occurs after an event with Terapagos.

// Var Settings
// To use the following features in scripting, replace the 0s with the var ID you're assigning it to.
// Eg: Replace with VAR_UNUSED_0x40F7 so you can use B_VAR_STARTING_STATUS for that feature.
#define B_VAR_STARTING_STATUS       VAR_TEMP_STARTING_STATUS    // If this var has a value, assigning a STATUS_FIELD_xx_TERRAIN to it before battle causes the battle to start with that terrain active.
...
#define B_VAR_SKY_BATTLE                  VAR_TEMP_SKY_BATTLE     // If this var has a value, the game will remember the positions of Pokémon used in Sky Battles.
...
#define B_CATCHING_CHARM_BOOST      25         // % boost in Critical Capture odds if player has the Catching Charm.
