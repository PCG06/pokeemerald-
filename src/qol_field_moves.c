...
{
    bool32 monHasMove = PartyHasMonLearnsKnowsFieldMove(ITEM_HM_CUT);
...
{
    bool32 monHasMove = PartyHasMonLearnsKnowsFieldMove(ITEM_HM_SURF);
    bool32 bagHasItem = CheckBagHasItem(ITEM_SURF_TOOL,1);
    bool32 playerHasBadge = FlagGet(FLAG_BADGE05_GET);
    bool32 collisionHasMismatch = ((collision == COLLISION_ELEVATION_MISMATCH) ? 1 : 0);
...
{
    bool32 monHasMove = PartyHasMonLearnsKnowsFieldMove(ITEM_HM_STRENGTH);
...
static void FieldCallback_UseFlashMove(void)
{
    ...
    PartyHasMonLearnsKnowsFieldMove(ITEM_HM_FLASH);
    gFieldEffectArguments[0] = gSpecialVar_Result;
...
u32 CanUseFlash(void)
{
    ...
    bool32 monHasMove = PartyHasMonLearnsKnowsFieldMove(ITEM_HM_FLASH);
...
{
    bool32 monHasMove = PartyHasMonLearnsKnowsFieldMove(ITEM_HM_ROCK_SMASH);
...
{
    bool32 monHasMove = PartyHasMonLearnsKnowsFieldMove(ITEM_HM_WATERFALL);
...
{
    bool32 monHasMove = PartyHasMonLearnsKnowsFieldMove(ITEM_HM_DIVE);
